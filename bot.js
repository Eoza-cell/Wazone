...
async function generateProfileImage(player) {
    ...
    const baseImagePath = `https://www.wazone.dev/images/base_${player.gender}.png`;
    ...
    for (const slot in player.equipment) {
        if (player.equipment[slot]) {
            ...
            if (equipmentItem && equipmentItem.image) {
                composites.push({ input: equipmentItem.image });
            }
        }
    }
    ...
    try {
        ...
        let image = sharp(Buffer.from(baseImageBuffer));
        ...
        // Retourner une image par défaut ou un message d'erreur
        return "https://www.wazone.dev/images/error.png";
    }
}
